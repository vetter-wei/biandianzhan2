...
import nari.app.BianDianYingYong.adapter.PJXXEditAdapter;
...
public class PJXMShowAdapter extends ArrayAdapter<PJXMBean> {

    public PJXMShowAdapter(Context context, List<PJXMBean> vos){
    ...
    public View getView(final int position,  View convertView, ViewGroup parent) {
        final PJXMShowAdapter.ViewHold vh;

        convertView = LayoutInflater.from(getContext()).inflate(R.layout.item_pjxm, null);
        vh = new PJXMShowAdapter.ViewHold(convertView);
        ...
        if(bean.getPjxxBeanList() == null || bean.getPjxxBeanList().size() == 0){
            ...
            if(bean.isChecked()){
                PJXXShowAdapter adapter = new PJXXShowAdapter(mContext,bean.getPjxxBeanList());
